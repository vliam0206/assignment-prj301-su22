...
//    private final String SHOPPING_PAGE="shoppingPage";
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        String url=siteMaps.getProperty(MyAppConstants.ShowProductFeatures.SHOPPING_PAGE);
        try {
            //1. call DAO
            ProductDAO dao = new ProductDAO();
            dao.loadAllProducts();           
            //2 process result
             List<ProductDTO> dto = dao.getProducts(); // neu dto = null thi sao? -> xu ly ben shopping.jsp
            request.setAttribute("PRODUCT_LIST", dto);            
        } catch (SQLException ex) {
            log("ShowProductList _ SQL "+ex.getMessage());
        } catch (NamingException ex) {
            log("ShowProductList _ Naming "+ex.getMessage());
        } finally {
